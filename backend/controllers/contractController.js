        ...
        pythonProcess.on('close', async code => {
            ...
            try {
                ...
                try {
                    ...
                    const requiredFields = ['contract_number', 'contracting_company', 'contracted_company'];
                    const missingFields = requiredFields.filter(field => !result.extracted_data[field]);

                    const status = missingFields.length > 0 ? 'Denegado' : 'Aceptado';
                    const validation_errors = missingFields.length > 0 
                        ? missingFields.map(field => `Campo requerido no encontrado: ${field}`)
                        : [];
                    ...
                    const updateData = {
                        ...
                        status,
                        validation_errors,
                        ai_decision_explanation: missingFields.length > 0 
