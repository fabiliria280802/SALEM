...
const fileFilter = (req, file, cb) => {
    ...
    if (!documentType || !allowedTypes[documentType]) {
        ...
        cb(new Error(`Tipo de documento no válido. Recibido: ${documentType}. Body: ${JSON.stringify(req.body)}`));
        return;
    }
    ...
    if (allowedTypes[documentType].includes(ext)) {
        ...
        cb(new Error(`Tipo de archivo no permitido para ${documentType}. Solo se permiten: ${allowedTypes[documentType].join(', ')}`));
    }
};
