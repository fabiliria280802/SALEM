describe('Flujo de carga y validación de contrato', () => {
	...
	it('Debería cargar y validar un contrato correctamente', () => {
		...
		cy.fixture('contract-sample.pdf', 'binary')
			...
			.then(fileContent => {
				const file = new File([fileContent], 'contract-sample.pdf', {
		...
		cy.get('.extracted-data').within(() => {
			cy.get('[data-testid="contracting-company"]').should(
				'contain',
				'ENAP SIPETROL S.A.',
			);
			cy.get('[data-testid="contract-number"]').should(
				'contain',
				'contract-001',
			);
		});
	...
	it('Debería manejar errores de validación', () => {
		...
		cy.get('.p-toast-message-error')
			...
			.and(
				'contain',
				'El RUC no está registrado o no pertenece a un proveedor',
			);
		...
		cy.fixture('invalid-file.txt', 'binary')
			.then(Cypress.Blob.binaryStringToBlob)
			.then(fileContent => {
				const file = new File([fileContent], 'invalid-file.txt', {
					type: 'text/plain',
				});
				const dataTransfer = new DataTransfer();
				dataTransfer.items.add(file);
