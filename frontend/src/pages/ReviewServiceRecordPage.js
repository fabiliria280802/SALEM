...
const ReviewServiceRecordPage = () => {
    ...
    const handleApprove = async () => {
        try {
            await documentService.updateDocument(id, 'ServiceDeliveryRecord', { status: 'Accepted' });
            toast.current.show({
                severity: 'success',
                summary: 'Éxito',
        ...
        <div className={styles.container}>
            ...
            <div className={styles.leftColumn}>
                ...
                <p className={styles.info}>
                    RUC: {documentData.ruc}<br />
                    Contrato: {documentData.contrato}<br />
                    Tipo de documento: {documentData.tipoDocumento}<br />
                        ...
                        {documentData.validationFields?.map((field, index) => (
                            <tr key={index}>
                                ...
                                <td className={styles.tableCell}>
                                    {documentData.validation_errors?.includes(field.field) ? 'No' : 'Sí'}
                                </td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                <p><strong>Descripción del error:</strong> {documentData.ai_decision_explanation}</p>
                <p className={styles.status}>
                    Estado: <span className={documentData.status === 'Denegado' ? styles.denied : styles.accepted}>
                        {documentData.status}
                    </span>
                </p>
                <Button 
